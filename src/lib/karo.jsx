...
function readMeId() {
  ...
  if (url) {
    try { localStorage.setItem('drumbeat.me', url) } catch {}
    return url
  }
  try { return localStorage.getItem('drumbeat.me') || null } catch { return null }
}
...
export function KaroProvider({ children }) {
  ...
  const [alias, setAlias] = useState(() => { try { return localStorage.getItem('drumbeat.alias') || null } catch { return null } })
  ...
  const rename = useCallback((name) => {
    ...
    setAlias(next)
    try { localStorage.setItem('drumbeat.alias', next) } catch {}
  }, [model])

  // identity: who the viewer is. Chosen via ?me=, the picker, or remembered.
  const setMe = useCallback((id) => {
    try { localStorage.setItem('drumbeat.me', id) } catch {}
    setMeId(id)
  }, [])
  const switchMe = useCallback(() => {
    try { localStorage.removeItem('drumbeat.me'); localStorage.removeItem('drumbeat.alias') } catch {}
    setAlias(null)
    setMeId(null)
  }, [])
  const meChosen = !!meId
